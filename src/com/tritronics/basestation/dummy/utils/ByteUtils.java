package com.tritronics.basestation.dummy.utils;
...
public class ByteUtils {
	...
	public static byte[] getBytes(short[] data) {
		...
		for(int i = 0; i < data.length; i ++){
			ba[i] = getBytes(data[i]);
		}
		
		return combineBytes(ba);
	}
	...
	public static byte[] combineBytes(byte[]... arrays) {
		...
		for(int i = 1; i < arrays.length; i++){
			...
			byte[] target = new byte[result.length + arrays[i].length];
			System.arraycopy(result, 0, target, 0, result.length);
			System.arraycopy(arrays[i], 0, target, result.length, arrays[i].length);
